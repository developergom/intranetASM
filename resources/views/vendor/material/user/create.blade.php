        ...
        <div class="card-body card-padding">
        	<form class="form-horizontal" role="form" method="POST" action="{{ url('user') }}">
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-10">
	                    <div class="fg-line">
	                        <select name="publisher_id[]" id="publisher_id" class="selectpicker" data-live-search="true" multiple>
                                @foreach ($publishers as $row)
                                	{!! $selected = '' !!}
                                	@if(old('publisher_id'))
	                                	@foreach (old('publisher_id') as $key => $value)
	                                		@if($value==$row->publisher_id)
	                                			{!! $selected = 'selected' !!}
	                                		@endif
	                                	@endforeach
                                	@endif
