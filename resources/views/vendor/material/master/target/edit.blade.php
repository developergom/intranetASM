...
@section('content')
    <div class="card">
        <div class="card-header"><h2>Target Management<small>Edit Target</small></h2></div>
        <div class="card-body card-padding">
        	<form class="form-horizontal" role="form" method="POST" action="{{ url('master/target/' . $target->target_id) }}">
        		{{ csrf_field() }}
	                    ...
	                    <div class="fg-line">
	                        <select name="industry_id" id="industry_id" class="selectpicker" data-live-search="true" required="true">
	                        	...
	                        	@foreach ($industries as $row)
                                	{!! $selected = '' !!}
                                	@if($row->industry_id==$target->industry_id)
                                		{!! $selected = 'selected' !!}
                                	@endif
	                ...
	                <div class="col-sm-10">
	                    <div class="fg-line">
	                        <select name="target_month" id="target_month" class="selectpicker" data-live-search="true" required="true">
	                        	<option value=""></option>
	                        	<option value="01" {{ ($target->target_month=='01') ? 'selected' : '' }}>January</option>
	                        	<option value="02" {{ ($target->target_month=='02') ? 'selected' : '' }}>February</option>
	                        	<option value="03" {{ ($target->target_month=='03') ? 'selected' : '' }}>March</option>
	                        	<option value="04" {{ ($target->target_month=='04') ? 'selected' : '' }}>April</option>
	                        	<option value="05" {{ ($target->target_month=='05') ? 'selected' : '' }}>May</option>
	                        	<option value="06" {{ ($target->target_month=='06') ? 'selected' : '' }}>June</option>
	                        	<option value="07" {{ ($target->target_month=='07') ? 'selected' : '' }}>July</option>
	                        	<option value="08" {{ ($target->target_month=='08') ? 'selected' : '' }}>August</option>
	                        	<option value="09" {{ ($target->target_month=='09') ? 'selected' : '' }}>September</option>
	                        	<option value="10" {{ ($target->target_month=='10') ? 'selected' : '' }}>October</option>
	                        	<option value="11" {{ ($target->target_month=='11') ? 'selected' : '' }}>November</option>
	                        	<option value="12" {{ ($target->target_month=='12') ? 'selected' : '' }}>December</option>
                            </select>
	                    </div>
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-2">
	                    <div class="fg-line">
	                        <input type="text" class="form-control input-sm" name="target_year" id="target_year" placeholder="Target Year ex: 2017" maxlength="4" value="{{ $target->target_year }}">
	                    </div>
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-4">
	                    <div class="fg-line">
	                        <input type="text" class="form-control input-sm" name="target_amount" id="target_amount" placeholder="Target Amount ex: 1000000" maxlength="15" value="{{ $target->target_amount }}">
	                    </div>
