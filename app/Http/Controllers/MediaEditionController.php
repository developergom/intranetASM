...
class MediaEditionController extends Controller
{
    ...
    public function apiList(Request $request, $id)
    {
        ...
        $data['rows'] = MediaEdition::join('medias','medias.media_id','=','media_editions.media_edition_id')
        ...
        $data['total'] = MediaEdition::join('medias','medias.media_id','=','media_editions.media_edition_id')
                            ->where('media_editions.active','1')
                            ->where('media_editions.media_id',$id)
                            ->where(function($query) use($searchPhrase) {
                                $query->where('media_edition_no','like','%' . $searchPhrase . '%')
